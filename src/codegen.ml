...
let translate (globals, functions) =
  ...
  let start_timer_t : L.lltype =
    L.function_type i32_t [|  i32_t |] in
  let start_timer_func : L.llvalue =
    L.declare_function "start_timer" start_timer_t the_module in

  let timer_destroy_t : L.lltype =
    L.function_type i32_t [|  i32_t |] in
  let timer_destroy_func : L.llvalue =
  ...
  let build_function_body fdecl =
    ...
    let rec expr builder ((_, e) : sexpr) = match e with
      ...
      | SCall ("timer_init", [e]) ->
        L.build_call timer_init_func [| (expr builder e)|] "timer_init" builder 

      | SCall ("start_timer", [e]) ->
        L.build_call start_timer_func [| (expr builder e)|] "start_timer" builder 

      | SCall ("timer_destroy", [e]) ->
        L.build_call timer_destroy_func [| (expr builder e)|] "timer_destroy" builder 
